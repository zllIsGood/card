 ...
 */
class DateUtils {
	/**时间格式 00:00:00 或 00:00 */
	public static TIME_FORMAT_0: number = 0;
	/**时间格式1 00:00:00 */
	...
	/**mode ='m'表示分 ='s'表示秒 ='ms'表示毫秒*/
	public static timeToString(time: number, mode = 'm') {
		if (mode == 'm') {
			let h = time / 60 << 0
			let m = (time - h * 60) << 0
			return (h + 'h' + m + 'm')
	...
	public static msToString(time: number) {
		let s = time / 1000 << 0
		if (s > 60 * 60) {
			return this.timeToString(s / 60, 'm')
		}
		else {
			return this.timeToString(s, 's')
		}
	}

	/**距离明天还有多少毫秒ms*/
	public static getSecondDayTime(): number {
		let ms = 0
		let time = new Date()
		ms = time.getHours() * 60 * 60 * 1000 + time.getMinutes() * 60 * 1000 + time.getSeconds() * 1000 + time.getMilliseconds()
		return ms
	}
	/**当前天数*/
