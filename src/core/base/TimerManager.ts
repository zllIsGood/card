...
class TimerManager extends BaseClass {
	private _handlers: Array<TimerComponent>;
	private _delHandlers: Array<TimerComponent>;
	...
	public constructor() {
		super();
		this._handlers = new Array<TimerComponent>();
		this._delHandlers = new Array<TimerComponent>();
	...
	private onEnterFrame(): void {
		...
		for (var i: number = 0; i < this._count; i++) {
			var handler: TimerComponent = this._handlers[i];
	...
	private removeHandle(handler: TimerComponent): void {
	...
	private create(useFrame: boolean, delay: number, repeatCount: number, method: Function, methodObj: any, complateMethod: Function, complateMethodObj: any): void {
		...
		//创建
		var handler: TimerComponent = ObjectPool.pop("TimerComponent");
	...
	public deleyPromisse(deley: number, thisObj = null): Promise<any> {
		let ms = deley < 0 ? 0 : deley
		return new Promise((resolve, reject) => {
			TimerManager.ins().doTimer(ms, 1, () => {
				resolve()
				// reject()
			}, thisObj)
		});
